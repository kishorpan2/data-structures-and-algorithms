...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((valueSofar, cur)=>{
    cur=cur.reduce((valueSofar,cur)=> valueSofar+cur);
    return valueSofar+cur;
  },0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map(individualArr=>{
    let newArray = individualArr.filter(arEl=>{
      if(typeof(arEl) === 'number'){
        if(arEl % 5 === 0){
          return true;
        }
      }
    });
    return newArray.map(arEl =>Math.pow(2, arEl));
  });
};
...
let findShortest = (data) => {
  // Solution code here...
  let shortPeopleData = data.reduce( (sumSoFar, current) =>
    sumSoFar.height < current.height ? current : sumSoFar);
